package kyle.memberapplication.helper;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // 데이터베이스 초기화 변수
    // device Explorer >> data >> data >> app >> databases
    private static final String DBNAME = "android.db";
    private static final int DBVERSION = 1;


    // 생성자 - 클래스 호출시 자동으로 sqlite db file 생성 (한번만)
    public DatabaseHelper(@Nullable Context context) {
        super(context, DBNAME, null, DBVERSION);
    }

    // db file 생성시 자동으로 실행하는 메서드
    // 주로 테이블 생성시 사용
    @Override
    ...
    public List<String> getAllUsers() {
        String sql = "select userid, name, email from member order by regdate desc";
        SQLiteDatabase db = this.getReadableDatabase();
